...
### For details about the project please refer to our project paper TODO Link
...
## Sampling
If you wish to sample from a pretrained model, download the [checkpoint]() and store it in a ```models``` folder. Use the ```test.py``` script to run the inference and sample from the pretrained video diffusion model. The script saves the sampled images in a ```diffused_frames``` folder.
